    ...
    <section id="contact" className="bg-zinc-950 py-24 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-zinc-500 text-base mb-12 max-w-xl">
          Hai un progetto in mente, una collaborazione da proporre, o vuoi semplicemente fare una chiacchierata?
          Trovi qui tutti i modi per contattarmi.
        </p>
        ...
        <div className="grid sm:grid-cols-3 gap-5">
          {socials.map(s => (
            <a
              ...
              className="group flex items-center gap-4 p-5 rounded-2xl bg-zinc-900 border border-zinc-800 hover:border-zinc-600 transition-all hover:-translate-y-0.5"
            >
              <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${s.gradient} flex items-center justify-center text-white flex-shrink-0 opacity-90 group-hover:opacity-100 transition-opacity`}>
                {s.icon}
              </div>
              <div>
                <p className="text-white font-medium text-sm">{s.name}</p>
                <p className="text-zinc-500 text-xs mt-0.5">{s.handle}</p>
              </div>

      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          <div className="space-y-5 text-zinc-400 text-base leading-relaxed">
            ...
            <a
              href="#contact"
              className="inline-flex items-center gap-2 text-amber-400 hover:text-amber-300 font-medium transition-colors mt-2"
            >
              {t('about.cta')}
          ...
          <div className="grid grid-cols-3 gap-4">
            {stats.map(s => (
              <div
                key={s.label}
                className="flex flex-col items-center justify-center p-5 rounded-xl bg-zinc-800 border border-zinc-700 text-center"
              >
                <span className="text-2xl font-bold text-amber-400 mb-1">{s.value}</span>
                <span className="text-zinc-500 text-xs font-medium uppercase tracking-wider">{s.label}</span>
              </div>
            ))}

            <div className="col-span-3 rounded-xl bg-zinc-800 border border-zinc-700 p-4 flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-zinc-700 flex items-center justify-center flex-shrink-0">
                <svg width="18" height="18" fill="none" viewBox="0 0 24 24" className="text-zinc-400">

...
function WorkIcon() {
  return (
    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" className="text-violet-400">
...
function EduIcon() {
  return (
    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" className="text-fuchsia-400">
...
export default function Experience() {
  ...
  const items = [
    ...
    },
    {
      ...
      desc: t('exp.edu.desc'),
      tags: ['Java', 'Reti', 'Sistemi', 'Programmazione'],
      current: false,
    },
  ]
          ...
          <div className="space-y-6">
            {items.map((item, i) => (
              <div key={i} className="md:pl-16 relative">
                {/* Icon on the line */}
                <div className={`absolute left-0 top-5 w-10 h-10 rounded-full border-2 items-center justify-center hidden md:flex ${
                  item.type === 'edu'
                    ? 'bg-fuchsia-600/20 border-fuchsia-500/40'
                    : 'bg-violet-600/20 border-violet-500/40'
                }`}>
                  {item.type === 'edu' ? <EduIcon /> : <WorkIcon />}
                </div>

                <div className={`rounded-2xl border p-6 transition-colors hover:border-zinc-600 ${
                  item.current
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
                    <div>
                      ...
                      <p className={`font-medium text-sm mt-0.5 ${item.type === 'edu' ? 'text-fuchsia-400' : 'text-violet-400'}`}>
                        {item.company}
                      </p>
                    </div>
